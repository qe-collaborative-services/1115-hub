    ...
    return `derived_from_cte AS (
      ...
      FROM (
          SELECT
              ...
              CASE WHEN scr.ENCOUNTER_ID IS NOT NULL THEN scr.ENCOUNTER_ID ELSE CONCAT('encounter-',scr.FACILITY_ID,'-',scr.PAT_MRN_ID)	END AS encounter_id,
              CASE WHEN scr.ENCOUNTER_ID IS NOT NULL THEN sha256(CONCAT('Observation/','observationResponseQuestion-',scr.ENCOUNTER_ID,'-',md5(scr.RECORDED_TIME),'-',acw_sub.QUESTION_SLNO)) ELSE sha256(CONCAT('Observation/','observationResponseQuestion-',scr.PAT_MRN_ID,'-',scr.FACILITY_ID,'-',md5(scr.RECORDED_TIME),'-',acw_sub.QUESTION_SLNO)) END  AS derived_reference
          FROM
              ahc_cross_walk acw
        ...
        'resource', JSON_OBJECT(
          'resourceType', 'Observation',
              ...
              'hasMember', (SELECT json_group_array(JSON_OBJECT(
                              'reference', CASE WHEN sub1.ENCOUNTER_ID IS NOT NULL THEN sha256(CONCAT('Observation/','observationResponseQuestion-',sub1.ENCOUNTER_ID,'-',md5(sub1.RECORDED_TIME),'-',sub1.QUESTION_SLNO)) ELSE sha256(CONCAT('Observation/','observationResponseQuestion-',sub1.PAT_MRN_ID,'-',sub1.FACILITY_ID,'-',md5(sub1.RECORDED_TIME),'-',sub1.QUESTION_SLNO)) END
                          ))
                          FROM (
                          SELECT DISTINCT
